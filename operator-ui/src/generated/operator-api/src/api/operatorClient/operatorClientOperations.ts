...
export async function listGameRegistrations(
  ...
  if (+response.status === 200 && response.headers["content-type"]?.includes("application/json")) {
    return jsonGameRegistrationListResponseToApplicationTransform(response.body)!;
  }
  throw createRestError(response);
}
;
...
export async function uploadGameBundle(
  ...
  const httpRequestOptions = {
    headers: {},body: [createFilePartDescriptor("bundle", body.bundle)],
  };
  const response = await client.pathUnchecked(path).post(httpRequestOptions);
...
export async function createOrReviseBot(
  ...
  const httpRequestOptions = {
    headers: {},body: jsonBotRevisionRequestToTransportTransform(body),
  };
  const response = await client.pathUnchecked(path).post(httpRequestOptions);
