...
- Rule: game 実装と game 固有 verification 資産が ai-arena の管理対象から外れる段階では、その game 専用 spec file は ai-arena から削除する。削除 gate は canonical golden だけでなく、その game の fixture bot、WASM/Rust AI player、CI coverage まで移行完了を列挙してから閉じる
