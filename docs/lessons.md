...
## [2026-05-08] child repo に template が無ければ workspace root を見る

- **Mistake**: `ai-arena` repo 直下に `PULL_REQUEST_TEMPLATE.md` が無い時点で、PR body を手書きで埋める方へ倒しかけた
- **Pattern**: meta-repo 配下 child project の PR template fallback を repo 単体の視点で閉じてしまい、workspace root の canonical template を見に行かない
- **Rule**: child repo に PR template が見当たらない場合は、その child repo だけで結論にせず、まず `vibe-coding-workspace/.github/PULL_REQUEST_TEMPLATE.md` を確認してから代替 body を考える
- **Applied**: `ai-arena` PR 作成フロー、今後の workspace 配下 child repo 全般
