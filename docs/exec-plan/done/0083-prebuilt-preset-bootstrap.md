...
- current staging backend は `ARENA_SERVICE_PRESET_CONFIG=./config/platform-service/presets.example.json`
  を読み、`preset_id = "echo-reference"` を queue へ積み、その player artifact として
  `./testdata/ai/echo/echo-ai-2turn` を参照している
- `./testdata/ai/echo/echo-ai-2turn.arena.json` は
  `runtime.command = ["go", "run", "./testdata/ai/echo/echo-ai"]`
...
### `docs/development/platform-service-object-storage.md`
...
- [ ] current remote preset chain (`presets.example.json` -> `preset_id = "echo-reference"` -> `./testdata/ai/echo/echo-ai-2turn` -> `go run`) を docs と code 上で固定し、差し替え target を明確にする
