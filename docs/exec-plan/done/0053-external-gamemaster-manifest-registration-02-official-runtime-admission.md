...
- [x] official game master runtime kind の候補を列挙する
- [x] executable / Docker / WASM-WASI / future external adapter を比較する
- [x] dev-only overlay と official registration policy の境界を定義する
- [x] official registration に必要な metadata / admission surface を定義する
- [x] ADR に採用方針と却下理由を記録する
...
- official admission は `official built-in` / `official sandboxed submission` / `official external adapter` の
  3 tier に分ける
- `official sandboxed submission` の第一候補 runtime kind は `wasm-wasi` とする
- simple executable / local subprocess は dev-only overlay に留め、official registration では許可しない
- `docker` / OCI container は将来候補として残すが、network isolation / resource limit / image scan などの
  admission 条件を満たす具体例が出るまで未サポートとする
