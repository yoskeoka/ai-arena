...
- (MODIFY) `typespec/namespaces/public/api.tsp`: list query model と paged response を wire-contract の正本として追加する。
...
- (MODIFY) `internal/platform/service/public_state.go`: selected public record を filter、sort、tie-break、page して、total を含む page result を組み立てる。
...
3. public read service で selected logical match を作った後に filter し、completed timestamp と `match_id` の全順序で sort してから page を切り出す。page metadata の `total` は filter/sort 後、page 前の件数とする。
...
5. service deploy workflow により staging をこの変更と既存 public metadata を含む SHA へ更新する。`/version`、scoped first page、detail、replay を credential-free request で確認し、historic record の provenance/replay が unavailable の場合は新しい completed Reversi match で acceptance evidence を取る。
...
- stg の旧 SHA 更新は API implementation の merge 後に行う。同一 API contract に依存する UI 実装は、mocked response/tests を先行してよいが、実 service acceptance は deploy 後に直列で行う。
...
- stg で `game_id=reversi&game_version_major=1&page=1&limit=20&sort=completed_at&sort_order=desc` が only Reversi major-1 records と page metadata を返し、new completed Reversi record が visualizer required metadata と `reversi/replay` を返すことを確認する。
