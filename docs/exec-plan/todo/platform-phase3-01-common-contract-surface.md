...
- 既存 spec に残る `turn_mode` 記述を洗い出し、互換性 metadata ではなく runtime step または game 分類タグへ寄せる更新方針を明記する
...
- `turn_mode` はこの plan で共通 metadata から外す方向で整理する
  ...
  - 既存 `docs/specs/janken-game.md` や `docs/specs/platform.md` 上の `turn_mode` 記述は、この phase で metadata key から runtime/description へ再分類する
...
- `DecisionMode` は metadata ではなく runtime step contract に残す
  - `DecisionStep.requests` の fan-out と矛盾しないよう、実行形態の明示または validation 用語として使う
