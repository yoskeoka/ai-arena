...
- game 固有の fixture bot / local AI / WASM AI / deterministic golden など、sidecar と同じ game ownership に属する
  verification asset は ai-arena の責務ではない。どの repo で管理するかは game 開発側の設計に委ねる
...
- ai-arena 側が継続検証する責務を持つのは `gamemaster` package と runner / runtime contract までとする
- game 固有 verification line の canonical source や asset 配置は、game 開発側 repo の判断対象であり、ai-arena が規定しない
