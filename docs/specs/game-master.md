...
`rng_seed` は string とする。game 固有に未使用でも受け取れてよい。dungeon phase では、この値を
`snapshot.game_state` と `exported_snapshot.public_state` に保持し、将来の seed 付き map
生成へそのまま拡張できる前提を取る。外部契約を string にする理由は、人間が replay / debug / CI で
再利用しやすく、seed 命名を integer format に縛らないためである。
