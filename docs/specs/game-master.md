...
## official runtime kind と admission policy

game master の開発入口と official registration policy は分離して扱う。
`local-subprocess` は consumer repo が tagged runner で fresh run を自己検証するための
dev-only 入口としては有用だが、そのまま official registration へ昇格させる前提にはしない。

official tier ごとの基本方針は次とする。

- `official built-in`
  - 運営自身が開発した source、または信頼できるパートナーから提供されて運営が review / release 管理する source を
    platform 管理下へ取り込む
  - runtime kind は `in-process` を基本としてよい
- `official sandboxed submission`
  - source を取り込まず、sandboxed runtime に載せた official game として扱う
  - 第一候補は `wasm-wasi` とする
  - `local-subprocess` executable は host filesystem / process 権限へ近く、network 制御も別途必要になるため、
    この tier では許可しない
...
filesystem / capability 制限、resource limit、image / dependency scan を admission 条件として要求する。
これらの制約を満たす具体例が出るまでは、spec 上は未サポート候補として扱う。
...
たとえば turn-based game では action normalization -> game-specific resolution phases ->
terminal / score update -> visible-state refresh の順を deterministic に適用してよい。
