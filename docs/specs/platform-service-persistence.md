...
この spec の対象は service orchestration が読む write path であり、operator-facing query API や
replay / resume 用の read model 詳細は別 plan で扱う。
...
single logical queue authority の最初の到達点は、複数 process が同じ backend を共有しても
1 queued record が同時に複数 worker へ lease されないことである。multi-node fairness や retry redelivery はこの spec の対象外とする。
