...
Phase 3 時点では、既存 repo 内 game master は `in-process` として登録してよい。

## `BuildSpec` 契約

descriptor の各 build 入口へ渡す最小入力は `BuildSpec` とし、少なくとも以下を含む。

- `GameVersion`
- `Ruleset`
- `Players`

snapshot resume build ではこれに加えて snapshot を受け取り、history replay build では event 列と
target turn を受け取る。
...
3. 各 game の build 入口が `ruleset_version` 妥当性を検証し、最終 metadata を持つ game master または snapshot を返す
...
- `BuildFresh(BuildSpec) -> game.Master`
- `BuildFromSnapshot(BuildSpec, snapshot) -> game.Master`
