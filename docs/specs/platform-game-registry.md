...
- `DefaultMode`
- `SupportedModes`
- selected mode を受け取る session build
...
descriptor は少なくとも 1 つの `DefaultMode` を持つ。必要なら複数 `SupportedModes` を持ってよい。
Phase 3 時点では、既存 repo 内 game master は `in-process` だけで登録してよい。
`echo-count` のような fixture game は `in-process` と `local-subprocess` の両方を持ってよい。
...
selected mode ごとの session build は、fresh run ではこの `BuildSpec` を受け取り、
...
3. 各 game の build 入口が `ruleset_version` 妥当性を検証し、selected mode で起動可能な game master session または snapshot を返す
...
- `BuildSession(mode, BuildSpec) -> game master session`
- `BuildSessionFromSnapshot(mode, BuildSpec, snapshot) -> game master session`
