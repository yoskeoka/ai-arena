...
- local/CI/release の schema apply / migration apply / query generation 入口
...
- versioned migration を replay する deploy lane では、
  runtime 用接続とは別の migration/admin 用接続を使う
- schema change を含む PR は、desired schema SQL と generated migration SQL を
  同じ review surface に含める
...
- versioned migration SQL artifact を target DB へ apply する
