...
## Phase 4 Multi-language Evaluation Lane

`janken` は Go の supported path を基準に保ったまま、non-Go WASM/WASI module の evaluation lane も受け持つ。

Phase 4 時点の language boundary:

- `supported`: Go
- `experiment-only`: Rust
- `future candidate`: TypeScript, Python

Rust evaluation lane の最小構成:

- game: `janken`
- game_version: `2.1.0`
- ruleset_version: `regular`
- players:
  - `p1=./testdata/ai/janken/janken-rust-wasm-ai`
  - `p2=./testdata/ai/janken/janken-rock-ai-wasm` または同等の比較用 fixture
...
- match は `completed` で終了する
- standard artifact として `record.json` / `history.json` が `<output-dir>/<match-id>/` に残る
- Rust-WASM player の `stderr_bytes` が 0 より大きく、runtime 経由で `stderr` capture が効いている
- mixed runtime 構成でも `public_history` / placement / failure 分類 contract が崩れない

運用境界:

- この lane は support 判定のための evaluation path であり、official support を意味しない
- Rust の詳細な build/runtime 観測は恒久 spec へ固定せず、helper 出力や appendix / issue note に分離してよい
- TypeScript / Python は将来候補として意図だけ残し、この spec では sample path を定義しない
