...
- `--exported-snapshot-output <target>` は file path または `stdout` を受け付ける
...
- `--history-input <path>` は persisted final record から抽出した history file を受け付ける
- `--record-input <path>` は persisted final match-record artifact を受け付ける
- `--target-turn <n>` は history replay / resume の turn 境界を指定する
