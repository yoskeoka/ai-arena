...
- `docs/specs/dungeon-game.md`: 本命 game の fixed-map MVP payload / validation / scoring

platform は game ごとの internal domain model を直接前提にしない。たとえば dungeon では
...
Phase 5 の dungeon MVP では、game master と reference bot の最速検証経路として
Go local subprocess を使ってよい。ただしこれは開発用 reference path であり、
正式提出経路の正本は引き続き WASM runtime contract に置く。

dungeon の manual verification では、単に到達順位を確認するだけでは不十分とする。
少なくとも「最短ゴール優先の line」と「宝箱回収を優先する treasure-heavy line」の両方で
score 差が発生し、宝箱回収が実際に逆転条件として機能することを確認対象に含める。
...
- same-condition regression が壊れた場合は、同一条件下の non-deterministic drift としてまず test failure 扱いにする。golden 更新を許可するのは、`game_version` / `ruleset_version` の意図的更新、deterministic AI 実装の意図的変更、normalized result shape 自体の見直し、または external repo が意図的に採用する ai-arena runner / platform version を上げた場合に限り、PR と spec で変更理由を明示する
