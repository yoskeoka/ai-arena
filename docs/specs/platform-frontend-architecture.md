...
current deploy shape は `Cloudflare Pages` frontend と `Render` backend の split-origin なので、
first landing の browser auth では backend-origin cookie + credentials 付き fetch を許容してよい。

- login entry page は frontend route として持ってよい
- OAuth callback 自体は backend origin に置いてよい
- frontend は login 完了後の return route と session-aware route guard を持ってよい
