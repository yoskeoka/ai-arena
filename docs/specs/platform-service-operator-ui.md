...
dedicated CI browser lane も同じ acceptance surface を共有しなければならない。
local lane が fixture-seeded backend を使ってよい一方で、
CI lane は actual operator API request により preset queue から started/completed state を作り、
同じ panel / detail / artifact observation surface を検証してよい。
