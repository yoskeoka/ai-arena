...
- `rng_seed` は 16 byte の seed material を表す 32 桁 hex string とする
- fresh run で caller が `rng_seed` を省略してよい場合、runner が新しい seed を生成して match を起動し、その値を snapshot / exported snapshot / record に保存する
- replay / resume では保存済み `rng_seed` を再利用する
- game master は `rng_seed` を match 初期条件の正本として保持する
- 実装は hex string をそのまま 16 byte seed material として decode し、`math/rand/v2` の deterministic source に渡す
...
主目的はゴール到達、宝箱はルート選択に揺らぎを与える副目的とする。
...
- 全宝箱合計は 48 点で、2 位が宝箱を独占しても 1 位単独到達を上回らない
...
```json
{
  ...
  "rng_seed": "00112233445566778899aabbccddeeff",
  "turn": 4,
  "max_turns": 16,
  "tiles": [
    "#########",
