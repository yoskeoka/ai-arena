...
- `rng_seed` は string とする
- 空文字は許可しない
- game master は `rng_seed` を match 初期条件の正本として保持する
- 実装は `rng_seed` を stable hash で内部 seed material に正規化してから `math/rand/v2` を使う
- 同じ `game_version` / `ruleset_version` / `player_id` 順 / `rng_seed` なら、初期生成結果は必ず一致しなければならない

`rng_seed` は replay / debug / exported snapshot の人間可読性を優先し、外部契約では integer ではなく
string で扱う。内部 random source への変換方法は実装 detail だが、同じ文字列から常に同じ乱数列が
得られることは spec 契約に含む。
