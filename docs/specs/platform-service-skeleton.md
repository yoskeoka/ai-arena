...
この spec は spectator 向け public HTTP API ではない。
service / worker / runner 間で共有する internal contract と、
first remote lane の process topology 前提を固定する。
...
- first remote lane の backend process 責務分離

この spec が定義しないもの:

- public HTTP API
- operator-facing HTTP route の request/response schema 詳細
...
- operator entry は CLI-first から始めてよいが、
  first remote lane では `docs/specs/platform-service-operator-api.md` が定義する
  operator-facing HTTP API を同じ service command / query に接続してよい
...
first remote landing の backend process は、
operator-facing HTTP API と in-process worker loop を同じ長寿命 process に同居させてよい。
この同居は initial deployment shape であり、
separate worker service や external queue broker を support 済みとは扱わない。
...
- fixture local browser lane:
  contributor または AI agent が同一環境で fixture backend、frontend、browser automation を起動してよい。
  この lane は file-backed / in-memory baseline を default としてよい
- real local browser lane:
  contributor または AI agent が同一環境で actual backend、actual frontend、browser automation を起動してよい。
  queue/state backend は Postgres contract を正本にし、
  artifact backend は local object storage が bootstrap できるなら S3-compatible lane を優先し、
  できない場合だけ file-backed へ fallback してよい
...
最低限の artifact は screenshot、trace、backend log、frontend log とする。
artifact 保存先は repo-local ignored path に固定してよい。
...
- operator-facing HTTP handler:
  preset queue request、active/completed polling、completed detail query を受ける
- in-process worker loop:
  空 queue 時は polling interval ごとに claim を再試行し、
  claimed submission を `leased -> running -> persisting -> terminal` まで進める

HTTP handler は queue lifecycle を直接 mutate してはならない。
mutation は admission 済み submit と queued-only cancel に限り service command を通す。
worker loop だけが `leased` 以降の execution lifecycle を進める。
...
- schema source、migration artifact、generated query artifact の責務分離は
  `docs/specs/platform-service-postgres-stack.md` に従う
...
operator-facing query surface は `submit` / `run-once` / `submit-cancel` に続く
`list` / `get` / `read` を最小入口とし、
compact result summary は `result-summary.json` から、
detail artifact locator は queue record が保持する terminal summary から引く。
