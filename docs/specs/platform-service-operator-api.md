...
同じ backend process が repo-owned provider test double route を同居させてもよい。

- これは verification seam であり、public operator API family ではない
- provider test double route は local / CI bootstrap 時だけ有効でよい
...
- fixed tester account seed が必要な場合、
  same backend bootstrap がその責務を持ってよい
