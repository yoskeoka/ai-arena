...
- operated-service の受入証跡は、preset を介さない operator general lane で、同一の検証済み game / AI bundle bytes を filesystem と S3-compatible store の両方へ upload し、manifest-derived game activation、互換 AI bundle による bot revision、manual match request、queue acceptance、worker materialization、および game / AI WASI session start まで完走させる。少なくとも echo-count と janken の game family を同じ手順で確認する。完走した run/read model と ranking は accepted game digest と各 seat の accepted AI digest を保持し、temporary materialization directory を残さない。外部 game の release asset を検証する場合は、公開 URL と `SHA256SUMS` の pinned digest を照合してから同じ bundle-only runner input に渡す。shared staging へ regression fixture を upload してはならない。
