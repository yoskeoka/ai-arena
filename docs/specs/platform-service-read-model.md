...
ranking aggregate はこの per-match read model とは別 contract である。
ranking lifecycle は `result-summary.json` と persisted submission metadata を使って
cross-match snapshot を構築するが、
`list` / `get` / `read` 自体に ranking row や leaderboard view を混ぜてはならない。
