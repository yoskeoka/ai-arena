...
生成後は `atlas.sum` も同じ directory で更新し、
review/apply metadata として migration SQL と一緒に commit する。

`make postgres-migrate-apply` は versioned migration replay の入口であり、
release lane では target DB へ commit 済み migration SQL を順番に apply する。
...
`make postgres-migrate-set VERSION=<migration_version>` は、
...
release workflow 有効化前にこの target で current version を記録する。
...
3. generated migration SQL と `atlas.sum` を review/apply artifact として commit する
4. staging / production 反映前に `make postgres-migrate-apply` で target DB へ apply する

この lane では migration SQL を人間が review する。
desired schema SQL が正本であり、migration SQL はそこから導いた artifact である。
schema change PR では、desired schema SQL と generated migration SQL、
`atlas.sum` を同じ PR に含める。
release target DB がすでに同等 schema を手動適用済みなら、
first workflow run 前に `make postgres-migrate-set VERSION=<latest_version>` で baseline を揃える。
...
## Runtime DSN と Migration DSN

- Render runtime env:
  `ARENA_SERVICE_POSTGRES_DSN`
  - `arena-service` が読む pooled connection string
- GitHub Actions migration secrets:
  `NEON_STAGING_MIGRATION_DSN`
  `NEON_PRODUCTION_MIGRATION_DSN`
  - release workflow が `AI_ARENA_PG_MIGRATION_DSN` へ流し込む direct/admin connection string

runtime pooled DSN は service request / worker query をさばくための接続であり、
schema migration lane に再利用してはならない。
Neon の schema change apply は session-dependent operation を含みうるため、
Connect modal の direct connection string を migration/admin 用 secret として分離する。

## Neon Secret 取得手順

1. Neon Console で対象 project (`ai-arena-stg` または `ai-arena-prod`) を開く
2. 対象 branch (`staging` または `production`) の `Connect` を開く
3. runtime 用には pooled connection string を取得し、Render service env の
   `ARENA_SERVICE_POSTGRES_DSN` へ設定する
4. migration 用には direct connection string を取得し、GitHub Actions secrets の
   `NEON_STAGING_MIGRATION_DSN` または `NEON_PRODUCTION_MIGRATION_DSN` へ設定する
5. secret rotation 後は、staging release で `make postgres-migrate-apply` が通ることを
   先に確認してから production へ進める
