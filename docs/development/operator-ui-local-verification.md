...
canonical command surface は次を正本とする。
...
contributor / AI agent が辿る repo-owned command はこの節の既存 script を正本にする。

fixture local verification の canonical command は次でよい。
...
この lane は bundle admission 済みの registered game と admitted bot を使って match request を作成する。
...
auth-enabled GitHub regression lane の canonical command は次で起動してよい。
...
の中で確認してよい。
認証後の operator surface では `/operator/invites` から
`participant|developer|operator` の invite を発行してよい。
...
helper は first operator bootstrap の固定 role 発行に留め、
role 選択が必要な invite は `/operator/invites` で扱う。
