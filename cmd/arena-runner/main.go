...
func run(args []string) error {
	...
	fs.StringVar(&exportedOutput, "exported-snapshot-output", "", "exported snapshot output target path or stdout")
	fs.StringVar(&recordInput, "record-input", "", "persisted final match-record input path")
	fs.StringVar(&snapshotInput, "snapshot-input", "", "snapshot input path")
	fs.StringVar(&historyInput, "history-input", "", "history input path")
	fs.IntVar(&targetTurn, "target-turn", 0, "history replay target turn boundary")
